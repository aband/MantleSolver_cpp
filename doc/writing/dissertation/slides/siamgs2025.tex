...
& && &&\Maroon{\text{/Compaction})}\\[8pt]
  		  &\nabla q - \nabla \cdot \tau(\mathbf{v}_s) &&= -(1-\phi)\rho_r\mathbf{g}, \quad && \Maroon{(\text{Stokes Flow})}\\
...
\begin{myslide}
\heading{Scaled Weak Formulation}
\subhead{Element averaged variable}:
...
\subhead{Scaled weak form} Modified to be locally mass conservative:
...
\begin{myslide}
\heading{FVM for General Conservation Law}
