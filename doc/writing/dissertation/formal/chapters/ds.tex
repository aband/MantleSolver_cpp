...
simplicity, we use the coordinate of corner $v_i$ in later computation.
We denote the coordinates of corner $v_i$ as $\mathbf{x}_{v,i}$.
...
\begin{equation}\label{eq:edgebasisds2}
		  \varphi^2_{e,i}(\mathbf{x}) = \frac{\lambda_{i+1}(\mathbf{x})\lambda_{i+3}(\mathbf{x})R_i(\mathbf{x})}
...
$\varphi^2_{e,0}$ on a general quadrilateral.
\begin{figure}[ht]
\centering\small
...
\begin{lem}\label{lem:interpolationop}
		  (Boundedness of the interpolation operator~\cite{directSerendipity})
...
\begin{lem}\label{lem:approximation}
		  (Approximability of the interpolation operator~\cite{directSerendipity})
