...
We formulate an overall explicit coupled solver as shown in algorithm~\ref{alg:coupled}.

The coupled solver is initialized from the initial distribution of the transport
variables $U^0$ specifying the initial mass concentration and enthalpy. The eutectic 
phase package gives $W^0$ corresponds to the $U^0$. 
Once we know $W^0$, we have phase related variables
porosity $\phi^n$ and individual concentrations $c_{i}^n$.

The next time step $U^{n+1}$ is obtained by marching in time explicitly.

The next time step $U^{n+1}$ is computed using fixed in time values for 
The effective velocities $\check{\mathbf{v}}_{\text{eff}}$ and phase 
averaged velocity $\check{\overline{\mathbf{v}}}$ are computed using 
