...
\caption{Preconditioned Inexact Uzawa Iteration}
		  \begin{algorithmic}[1]
					 ...
					 \State Update $\bold{y}_{i+1} = \bold{y}_i + \bold{Q}^{-1} (\bold{G} - \bold{B}^T\bold{x}_{i+1} - \bold{C}\bold{y}_{i})$
\end{algorithmic}
\end{algorithm}

In the previous literature \cite{AnalysisInexactUzawa_Bramble_Pasciak_Vassilev}, 
$\bold{Q}$ is approximated with $\tau \bold{I}$, which works very well with the 
stokes part. However, it converges very slow with the same approximation when 
applied to Darcy part. Hence we proposed a more accuate Uzawa iteration procedure.
...
\caption{Preconditioned Inexact Uzawa Iteration}
		  \begin{algorithmic}[1]
					 \State Initialize $\bold{x}_0 = \bold{0}$ and $\bold{y}_0 = \bold{0}$
					 \State Update $\bold{x}_{i+1} = \bold{x}_i + \bold{A}^{-1} (\bold{F} - (\bold{A}\bold{x}_i - \bold{B}\bold{y}_i))$
					 \State Obtain $\bold{r} = \bold{G} - \bold{B}^T\bold{x}_{i+1} - \bold{C}\bold{y}_{i}$
