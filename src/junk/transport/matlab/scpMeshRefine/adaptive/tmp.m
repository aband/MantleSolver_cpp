...
%refinement = refine;
%href = 0.1/refinement;
%x = [linspace(-1,-0.9,refinement),linspace(-0.9+href,0.9-href,M-2*refinement),linspace(0.9,1,refinement)];
...
[stencilPolyn32,stencilPolyn43] = createStencilPolyn(x);
...
for time = 1:NT
    ...
    % Create added resolution

    % refine it
    %N = size(refinedCell,2);
    %x = refinedx;
